...
class BuscadorCentroPoblado extends Component
{
    ...
    public function render()
    {
        ...
        if ($this->selectedDepartamento && $this->selectedProvincia && $this->search) {
            $tramits = Tramite::where('tecnico_id', auth()->user()->tecnico->id)
            ->where('departamento_id',$this->selectedDepartamento)
        ...
        elseif($this->selectedDepartamento && $this->selectedProvincia) {
            $tramits = Tramite::where('tecnico_id', auth()->user()->tecnico->id)
            ->where('departamento_id',$this->selectedDepartamento)
            ->where('provincia_id',$this->selectedProvincia)->get();
            ...
            return view('livewire.admin.buscador-centro-poblado',compact('tramits')); 
