<div class="max-w-5xl bg-white rounded-lg shadow-lg mx-auto px-4 py-2 sm:px-6 lg:px-8 mt-3">
    ...
    @else
        <div class="flex py-2">
            ...
            @role('tecnico')
                @livewire('entidades.create-vicemintierra', ['ritu' => $ritu], key($ritu->id))
            @endrole
        </div>
    @endif
    ...
    <div>
        <x-table-responsive>
            ...
            @if ($vicemintierras->count())
                <table class="min-w-full divide-y divide-blue-400">
                    ...
                    <tbody class="bg-white divide-y divide-blue-400">
                        ...
                        @foreach ($vicemintierras as $item)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="ml-2">
                                            @livewire('entidades.show-doc-vicemintierra', ['item' => $item], key($item->id))
                                        </div>
                                    </div>
                                </td>
                                @role('tecnico')
                                    <td class="px-2 py-4 whitespace-nowrap  text-sm font-medium">
                                        <div class="inline-flex rounded-md shadow-sm">
                                            ...
                                            <a href="#" class="btn btn-red cursor-pointer"
                                                wire:click="$emit('deleteVicemintierra',{{ $item->id }})">
            ...
            @if ($vicemintierras->hasPages())
                <div class="px-6 py-4">
                    {{ $vicemintierras->links() }}
                </div>
            @endif
    ...
    <x-jet-dialog-modal wire:model="open_edit">
        ...
        <x-slot name="content">
            ...
            <div class="mb-4">
                ...
                <x-jet-input type="text" class="w-full  mb-2" wire:model="vicemintierra.numruta" />
                <x-jet-input-error for="vicemintierra.numruta" />
            </div>

            <div class="mb-4">
                <x-jet-label value="Fecha de Emisión" />
                <x-jet-input type="date" class="w-full  mb-2" wire:model="vicemintierra.fecha" />
                <x-jet-input-error for="vicemintierra.fecha" />
            </div>

            <div class="mb-2 w-full">
                <x-jet-label value="Observación" />
                <textarea name="" id="" cols="10" rows="2" class="form-control w-full mb-2"
                    wire:model="vicemintierra.observacion"></textarea>
                <x-jet-input-error for="vicemintierra.observacion" />
            </div>
    ...
    @push('script')
        ...
        <script>
            Livewire.on('deleteVicemintierra', vicemintierraId => {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        Livewire.emitTo('entidades.show-vicemintierra', 'delete', vicemintierraId);
