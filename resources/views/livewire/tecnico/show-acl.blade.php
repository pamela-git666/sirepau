    ...
    @if ($tramite->fase > 3)
        ...
        <div class="bg-white rounded-lg shadow-lg px-12 py-4 mb-4 items-center w-auto">
            <div class="flex px-2 py-2 sm:px-6">
                <h2 class="text-center  text-2xl font-bold leading-6 text-blue-700">Análisis de Concordancia Legal </h2>
                @role('tecnico')
                    @livewire('tecnico.create-acl', ['tramite' => $tramite], key($tramite->id))
                @endrole
            </div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="ml-2">
                                                ...
                                                @livewire('tecnico.show-doc-acl', ['item' => $item], key($item->id))
                                            </div>
                                        </div>
                                    </td>
                                    @role('tecnico')
                                        <td class="px-2 py-4 whitespace-nowrap  text-sm font-medium">
                                            <div class="inline-flex rounded-md shadow-sm">
                                                ...
                                                <a href="#" class="btn btn-red cursor-pointer"
                                                    wire:click="$emit('deleteAcl',{{ $item->id }})">
        ...
        <x-jet-dialog-modal wire:model="open_edit">
            ...
            <x-slot name="content">
                ...
                <div class="mb-4">
                    ...
                    <x-jet-input type="text" class="w-full  mb-2" wire:model="acl.numruta" />
                    <x-jet-input-error for="acl.numruta" />
                </div>

                <div class="mb-4">
                    <x-jet-label value="Fecha de Inicio de Trámite" />
                    <x-jet-input type="date" class="w-full  mb-2" wire:model="acl.fecha" />
                    <x-jet-input-error for="acl.fecha" />
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select
                        ...
                        <option value="Informe de Análisis y Concordancia">Informe de Análisis y Concordancia</option>
                        <option value="Informe Legal">Informe Legal</option>
                    </select>
                    <x-jet-input-error for="acl.tipo" />
                </div>

                <div class="mb-2 w-full">
                    <x-jet-label value="Observación" />
                    <textarea name="" id="" cols="10" rows="2" class="form-control w-full mb-2"
                        wire:model="acl.observacion"></textarea>
                    <x-jet-input-error for="acl.observacion" />
                </div>
        ...
        <div class="flex bg-white rounded-lg shadow-lg px-12 py-4 mb-4 items-center w-auto">
            ...
            <div class="text-blue-600">
                <span class="font-bold text-lg text-blue-500">
                    No se encuentra habilitada la etapa de Análisis de Concordancia Legal.
                </span>
            </div>
        </div>
    @endif
        ...
        Livewire.on('deleteAcl', aclId => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    Livewire.emitTo('tecnico.show-acl', 'delete', aclId);
